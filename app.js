...
document.querySelectorAll('.selected-video-card').forEach(card=>{
 const video=card.querySelector('video'),cover=card.querySelector('.selected-cover'),error=card.querySelector('.selected-play-error');
 video.addEventListener('play',()=>{document.querySelectorAll('video').forEach(other=>{if(other!==video)other.pause()})});
 if(!cover)return;
 cover.addEventListener('click',()=>{cover.hidden=true;video.controls=true;error.hidden=true;video.focus();video.play().catch(()=>{error.hidden=false})});
 video.addEventListener('playing',()=>{error.hidden=true});
 video.addEventListener('ended',()=>{video.controls=false;cover.hidden=false});
 video.addEventListener('error',()=>{error.hidden=false;video.controls=true});
});
...
const videoWorks=[{"title":"《太平山雾顶》苏晓","src":"assets/suxiao-current/work-1.mp4","note":""},{"title":"【MV】分餐（粤语）","src":"assets/suxiao-current/work-2.mp4","note":""},{"title":"笼","src":"assets/suxiao-current/work-3.mp4","note":""},{"title":"世界杯","src":"assets/suxiao-current/work-4.mp4","note":""}];
const videoSection=document.createElement('section');videoSection.className='profile-videos';videoSection.hidden=true;
videoSection.innerHTML='<div class="profile-video-heading"><div><p class="eyebrow">SU XIAO / VIDEO DIARY</p><h3>镜头里的苏晓</h3></div><span>04 FILMS</span></div><div class="profile-video-grid"></div>';
ipDialog.append(videoSection);
const videoGrid=videoSection.querySelector('.profile-video-grid');
videoWorks.forEach((work,index)=>{const article=document.createElement('article');article.className='profile-video-card';const video=document.createElement('video');video.controls=true;video.playsInline=true;video.preload='none';video.dataset.src=work.src;video.setAttribute('aria-label',work.title);const fallback=document.createElement('a');fallback.href=video.dataset.src;fallback.textContent='打开视频';video.append(fallback);const title=document.createElement('h4');title.textContent=`0${index+1} / ${work.title}`;const note=document.createElement('p');note.textContent=work.note;const error=document.createElement('p');error.className='video-error';error.hidden=true;error.textContent='视频暂时无法播放，请检查文件是否完整，或使用支持 MP4 的浏览器。';video.addEventListener('error',()=>{error.hidden=false});video.addEventListener('play',()=>videoGrid.querySelectorAll('video').forEach(other=>{if(other!==video)other.pause()}));article.append(video,title,note,error);videoGrid.append(article)});
videoGrid.querySelectorAll('.profile-video-card').forEach((card,index)=>{
 const video=card.querySelector('video');const poster=videoWorks[index].src.replace(/\.mp4$/,'.jpg');video.poster=poster;video.controls=false;
 const frame=document.createElement('div');frame.className='suxiao-player';video.before(frame);frame.append(video);
 const button=document.createElement('button');button.type='button';button.className='selected-cover';button.setAttribute('aria-label','播放'+videoWorks[index].title);
 const image=document.createElement('img');image.src=poster;image.alt=videoWorks[index].title+'第一帧';const icon=document.createElement('span');icon.textContent='▶';icon.setAttribute('aria-hidden','true');button.append(image,icon);frame.append(button);
 button.addEventListener('click',()=>{button.hidden=true;video.controls=true;video.play().catch(()=>{button.hidden=false;video.controls=false})});
 video.addEventListener('ended',()=>{button.hidden=false;video.controls=false});
 ipDialog.addEventListener('close',()=>{button.hidden=false;video.controls=false});
});
